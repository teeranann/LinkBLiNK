function im = readSEQ(fileName,frames)
% -------------------------------------------------------------------------
...
im.ImageWidth = imageInfo(1);
im.ImageHeight = imageInfo(2);
im.ImageBitDepth = imageInfo(3);
im.ImageBitDepthReal = imageInfo(4);
im.ImageSizeBytes = imageInfo(5);
...
im.ImageFormat = fmts{vals == imageInfo(6)};
fseek(fid,572,'bof');
im.AllocatedFrames = fread(fid,1,'ushort',endianType);
fseek(fid,620,'bof');
im.Compression = fread(fid,1,'uint8',endianType);
% Additional sequence information
fseek(fid,28, 'bof');
im.HeaderVersion = fread(fid,1,'long',endianType);
fseek(fid,32,'bof');
im.HeaderSize = fread(fid,4/4,'long',endianType);
fseek(fid,592, 'bof');
DescriptionFormat = fread(fid,1,'long',endianType)';
fseek(fid,36,'bof');
im.Description = fread(fid,512,'ushort',endianType)';
if DescriptionFormat == 0 %#ok Unicode
    im.Description = native2unicode(im.Description);
elseif DescriptionFormat == 1 %#ok ASCII
    im.Description = char(im.Description);
end
fseek(fid,580,'bof');
im.TrueImageSize = fread(fid,1,'ulong',endianType);
fseek(fid,584,'bof');
im.FrameRate = fread(fid,1,'double',endianType);

% warn if image format is not tested
if strcmp(im.ImageFormat,'Monochrome')
    fprintf('Proceeding with image format %s.\n', im.ImageFormat)
elseif strcmp(im.ImageFormat,'BGR')
    fprintf('Proceeding with image format %s.\n', im.ImageFormat)
else
...
if exist('frames','var') == 1
    ...
    elseif frames(1) > im.AllocatedFrames || frames(2) > im.AllocatedFrames
        error(['Some values of selcted frames (' num2str(frames) ') are above allocated frames of ' num2str(im.AllocatedFrames) '.'])
...
if frames(2) <= 0, last = im.AllocatedFrames; else, last = frames(2); end
readAmount = last+1 - first;

if readAmount ~= im.AllocatedFrames
    loopEnd = readAmount;
else
    loopEnd = im.AllocatedFrames; % user selected no read window
end
im.imageraw = cell(readAmount,2); % create an empty cell to store images

% set bit depth for reading
switch im.ImageBitDepthReal
    case 8
        bitDepth = 'uint8';
    case {10,12,14,16}
...
if im.Compression == 1 % sequence is compressed
...
elseif im.Compression == 0 % sequence is uncompressed
    
    for i = 1:loopEnd
        readStart = ((first-1+i-1)*im.TrueImageSize)+8192; % set readStart to frame
        im.imageraw(i,1:2) = ReadUncompressedFrameWithoutIdx(fid, readStart, im, bitDepth);
    end
end
fclose(fid);
% chosenframe=(1:5); %number of frame needed
im.image=cat(3,im.imageraw{:,1}); %convert image to 3D matrix
return
...
fseek(fid,readStart,'bof');
ImageSize=headerInfo.ImageWidth*headerInfo.ImageHeight;
vec = fread(fid, ImageSize, bitDepth, endianType);

if strcmp(headerInfo.ImageFormat,'Monochrome')
    I{1,1} = uint16(reshape(vec,headerInfo.ImageWidth,headerInfo.ImageHeight)');
